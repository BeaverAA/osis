...
INT_PTR CALLBACK bout(HWND, UINT, WPARAM, LPARAM);

void CreateControls(HWND hWnd);
void AddToList(HWND hList, const string_t& s);
void ClearList(HWND hList);
vector<string_t> GetProcessModules(int processId);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
   ...
   HWND hWnd = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, 720, 640, nullptr, nullptr, hInstance, nullptr);
   if (hWnd == FALSE) {
      return FALSE;
   }
...
void ShowModulesForProcess() {
	...
	vector<string_t> modules = GetProcessModules(curProcess);
